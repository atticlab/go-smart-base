...
func encodeMnemonic(s string) string {
	var out []string
	n := int64(len(dict))
	nF := float64(n)
	for i := 0; i < len(s); i += 8 {
		x, _ := strconv.ParseInt(s[i:(i+8)], 16, 64)
		xF := float64(x)
		w1 := x % n
		w2 := int64(math.Floor(xF/nF) + float64(w1)) % n
		w3 := int64(math.Floor(math.Floor(xF/nF)/nF) + float64(w2)) % n
		out = append(out, dict[w1], dict[w2], dict[w3])
	}
...
func decodeMnemonic(s string) string{
	var out string
	n := len(dict)
	wList := strings.Split(s, " ")
	for i := 0; i < len(wList); i +=3 {
		w1 := indexOf(wList[i], dict)
		w2 := indexOf(wList[i+1], dict)
		w3 := indexOf(wList[i+2], dict)
		x := w1 + n * mod((w2 - w1), n) + n * n * mod((w3 - w2), n);
		outT := strconv.FormatInt(int64(x), 16)
		if len(outT) < 8 {
			for len(outT) < 8 {
				outT = "0" + outT
			}
			...
			out += (strconv.FormatUint(uint64(x), 16))
		}
